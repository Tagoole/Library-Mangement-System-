...
public class MemberController {
    ...
    public ResponseEntity<ApiResponse<MemberResponse>> create(@Valid @RequestBody MemberRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ApiResponse<>("SUCCESS", "Member created successfully", memberFacade.create(request)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<MemberResponse>> findById(@PathVariable Long id) {
        return ResponseEntity.ok(new ApiResponse<>("SUCCESS", "Member fetched successfully", memberFacade.findById(id)));
    }

    @GetMapping
    public ResponseEntity<ApiResponse<List<MemberResponse>>> findAll() {
        return ResponseEntity.ok(new ApiResponse<>("SUCCESS", "Members fetched successfully", memberFacade.findAll()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<MemberResponse>> update(@PathVariable Long id, @Valid @RequestBody MemberRequest request) {
        return ResponseEntity.ok(new ApiResponse<>("SUCCESS", "Member updated successfully", memberFacade.update(id, request)));
    }
